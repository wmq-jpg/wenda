...
public class FollowHandler implements EventHandler {
    ...
    public void doHandle(EventModel model) {
        ...
        if(model.getEntityType()== EntityType.ENTITY_USER) {
            message.setContent("用户" + user.getName() + "关注了你，http:127.0.0.1:8889:/user" + model.getActorId());
        }
        else if(model.getEntityType()==EntityType.ENTITY_QUESTION)
        {
            message.setContent("用户" + user.getName() + "关注了你的问题，http:127.0.0.1:8889:/question" + model.getEntityId());
        }
